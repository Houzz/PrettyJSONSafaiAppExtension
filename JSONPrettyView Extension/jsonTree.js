...
var jsonTree = (function() {
                    ...
                    str = '\
                        <div class="jsontree_value-wrapper">\
                            <div class="jsontree_value jsontree_value_' + self.type + '">\
                                ...
                                <div class="jsontree_child-nodes"></div>\
                                <b>' + sym[1] + '</b>' +
                            '</div>' + comma +
                        '</div>';
                    ...
                    str = '\
                        <span class="jsontree_label-wrapper">\
                            <span class="jsontree_label">' +
                                ...
                                (containsSpace(label) ? '' : '"') + label + (containsSpace(label) ? '' : '"') +
                            '</span>: \
                        </span>' + str;
                }
    ...
    utils.extend(_NodeComplex.prototype, {
        ...
        expand : function(isRecursive){
         ...
         limitedExpand : function(isRecursive, limit){
             if (this.isEmpty) {
                 return;
             }
             ...
             if (isRecursive) {
                 this.childNodes.forEach(function(item, i) {
                     if (item.isComplex && i < limit) {
                         item.limitedExpand(isRecursive, limit);
                     }
                 });
             }
         },
    ...
    Tree.prototype = {
        ...
        limitedExpand : function(limit) {
                this.rootNode.limitedExpand('recursive', limit);
        },
        expandOne : function() {
             this.rootNode.expandOne()
        },
